...
class CommissionsPerVendor extends Component {
  state = {
    data: {},
  ...
  populate = () => {
    axiosInstance
      .get(
        `/admins/reports/commissions_per_vendor?start_date=${this.state.start_date.format(
          "YYYY-MM-DD"
  ...
  render() {
    return (
      <Col md={12}>
        <Card>
          <CardBody>
            <CardTitle>Commissions Per Vendor</CardTitle>
            <CardText>
              <ReportFilterWidget
                run={this.run}
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(CommissionsPerVendor)
);
