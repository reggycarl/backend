...
class Request extends Component {
    ...
    setRequest = (state, request) => {
        console.log("THIS IS THE REQUES FROM SETREQ", request)
        this.setState({
            ...state,
            request: {
                searchingRider: _.isEmpty( request.partner ) ? true : false,
                from_location: {
    ...
    componentDidMount = ()=> {
        ...
        Promise.all([getVehicleTypes(), getPaymentMethods()]).then(([delivery_types, payment_methods]) => {
            console.log("THESE ARE PAYMENT METHODS", payment_methods);
            var new_state = {...this.state, 
                delivery_types: delivery_types,
                payment_methods: payment_methods,
            }
            if (this.state.existing_record){
                axiosInstance.get(`/users/requests/${this.state.request_uuid}`).then(response => {
    ...
    requestUpdated = msg => {
        console.log(msg);
        this.setRequest(this.state, msg.request)
        // this.setState({
        //     ...this.state,
        //     request: {
        //         ...this.state.request,
    ...
    requestDelivery = e => {
        ...
        if (self.state.readOnly == true && self.state.existing_record == true) {
            ...
            if (self.state.existing_record != true) {
                instance = axiosInstance.post('/users/requests', { ...this.state })
            }
            else {
                
                instance = axiosInstance.put(`/users/requests/${this.state.request_id || this.state.request.uuid}`, { request: this.state.request })
            }
            instance.then((response) => {
                ...
                if (this.state.existing_record == true) {
                    var new_state = this.setRequests(this.state, response.data.request)
                    console.log("THIS IS NEW STATE", new_state)
                    this.setState({ ...new_state, readOnly: true })
                }
                else {
                    this.props.history.push(`/users/requests/${response.data.request.uuid}`)
                            ...
                            <Col md={6}>
                                ...
                                <FormGroup className='col-md-12' >
                                            ...
                                            <Col md={2}>
                                                ...
                                                <Col md={10}>
                                            <Input type="text"   placeholder="Request Number"  readOnly={true} value={this.state.request.number}/>
                                            </Col>
                                            </Row>
                                </FormGroup>
                                        <FormGroup className='col-md-12' >
                                        ...
                                        <FormGroup className='col-md-12' >
                                            <Row>
                                                <Col md={2}>
                                                    <Label for="exampleEmail">Assigned To</Label>
                                                </Col>
                                                <Col md={10}>
                                                <Col id='partnerInfoBox'>
                                                {this.state.request.searchingRider ? <Col className='spinner'><Loader
                                                        ...
                                                        width={40}
                                                    />  
                                                    <p>Searching for Rider</p>
                                                    </Col>: 
                                                <Row>
                                                    <Col md={12}>
                                                        <Row>
                                                            <Col md={3} className='picBox'>

                                                            </Col>
                                                            <Col md={8} className="partnerInfo">
                                                                <h4>{this.state.request.partner.full_name}</h4>
                                                                <h6>{this.state.request.partner.phone}</h6>
                                                            </Col>
                                                        </Row>
                                                    </Col>
                                                </Row> }
                                    ...
                                    <Row>
                                        ...
                                        <Button onClick={this.cancelRequest} color="danger" size='lg' className='form-control' > Cancel  Request </Button> : 
                                        <Button onClick={this.requestDelivery} color="primary" size='lg' className='form-control' disabled={this.state.gettingPrice}> Request Delivery </Button> 
                                        }
                                    </Row>
                                </Col>
                                </Col>
                                <ActionCableConsumer
                                    ...
                                    onReceived={this.requestUpdated}
                                    onConnected={()=>{
